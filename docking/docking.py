...
camera.framerate = 30
rawCapture = PiRGBArray(camera, size=(RX, RY))

result = cv2.VideoWriter('demo.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (RX, RY))
nFrames = 0
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    #         print(avg_x, avg_y)
    #         print(avg_x - RX / 2, avg_y - RY / 2)
    #         if abs(avg_x - RX / 2) > RX * .05 and abs(avg_y - RY / 2) > RY * .05:
    #             cv2.drawContours(img, [approx], -1, (0, 0, 255), 1)
    #             cv2.arrowedLine(img, (int(RX / 2), int(RY / 2)), (int(avg_x), int(avg_y)), (0, 0, 255), 2)
    #         else: 
